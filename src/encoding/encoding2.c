...
inline static size_t objc_max_size(size_t v, size_t v2)
{
	return v>v2 ? v : v2;
}
...
static const char *sizeof_union_field(const char *type, size_t *size)
{
	...
	*size = objc_max_size(*size, field_size);
	return end;
}
...
#define APPLY_TYPE(typeName, name, capitalizedName, encodingChar) \
		case encodingChar:\
		{\
			*align = objc_max_size((alignof(typeName) * 8), *align);\
			return type + 1;\
		}
#define NON_INTEGER_TYPES 1
#define SKIP_ID 1
#include "type_encoding_cases.h"
		case '@':
		{
			*align = objc_max_size((alignof(id) * 8), *align);\
			if (*(type+1) == '?')
			{
				type++;
			}
...
#define APPLY_TYPE(typeName, name, capitalizedName, encodingChar) \
		case encodingChar:\
		{\
			*align = objc_max_size((alignof(_Complex typeName) * 8), *align);\
			return type + 1;\
		}
#include "type_encoding_cases.h"
			}
		...
		case '^':
		{
			*align = objc_max_size((alignof(void*) * 8), *align);
			// All pointers look the same to me.
			size_t ignored = 0;
			// Skip the definition of the pointeee type.
