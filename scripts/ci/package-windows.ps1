...
$runtimeDir = Join-Path $build "runtime\Release"
$library = Join-Path $runtimeDir "mosaic_objc_runtime.lib"
$configHeader = Join-Path $runtimeDir "objc\objc-config.h"
if (-not (Test-Path $library)) {
    throw "Runtime library not found: $library"
}
if (-not (Test-Path $configHeader)) {
    throw "Generated Objective-C config header not found: $configHeader"
}
...
Copy-Item $configHeader (Join-Path $stage "include\objc\objc-config.h")
